...
class Db {
    ...
    protected function debug() {
        ...
        if (C('DB_SQL_LOG')) {
//            G('queryEndTime');
//            trace($this->queryStr,'','SQL');
        	Log::write($this->queryStr,'SQL');
        }
    }
     ...
     */
    public function insert($data,$options=array(),$replace=false, $delayed=false) {
        $values  =  $fields    = array();
        ...
        $sql   =  ($replace?'REPLACE':'INSERT').($delayed?' DELAYED':'').' INTO '.$this->parseTable($options['table']).' ('.implode(',', $fields).') VALUES ('.implode(',', $values).')';
