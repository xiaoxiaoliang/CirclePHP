...
class Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        // 获取数据库操作对象
        // 当前模型有独立的数据库连接信息
        $this->db(md5($connection),empty($this->connection)?$connection:$this->connection);
    }
...
//    }
     ...
     * @param boolean $DELAYED 是否DELAYED
     * @return mixed
     */
    public function add($data='',$options=array(),$replace=false, $delayed=false) {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            if(!empty($this->data)) {
                $data           =   $this->data;
        ...
        // 写入数据到数据库
        $result = $this->db->insert($data,$options,$replace, $delayed);
        if(false !== $result ) {
